...
<script>
import Vue from 'vue'
import { getSysDictList, roleFindAll } from '@/api/workFlow/request.js'
...
export default {
  ...
  watch: {
    ...
    //     if (!val) return
    //     let total = this.getFormVal(this.bindPrice)
    //     if (this.value && total) {
    //       if (this.price.toFixed(2) !== Number(total).toFixed(2)) {
    //         return this.$notification['error']({
    //           message: '系统通知',
    //           description: '分摊金额的合必须等于总金额'
    //         })
