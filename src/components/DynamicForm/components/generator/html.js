...
const layouts = {
  colFormItem(element, noFormItem = false) {
    ...
    let rules = '',
      label = `label="${noFormItem ? '' : element.label}"`,
    ...
    const tableTitle = noFormItem
      ? `<div style="font-size: 14px;padding:6px 0px;margin-bottom: 4px;border-bottom: 1px solid #e5e5e5;">${element.label}</div>`
      : ''
    let str = `${tableTitle}
  ...
  rowFormItem(element) {
    ...
    const explain =
      element.cmpType === 'custom' && element.explain
        ? `
...
const tags = {
  ...
  'el-upload': el => {
    ...
    const onPreview = `:on-preview="onPreview"`
    ...
    if (child) child = `\n${child}\n` // 换行
    return `<${el.tag} ${ref} ${fileList} ${action} ${autoUpload} ${multiple} ${beforeUpload} ${onPreview} ${listType} ${accept} ${name} ${disabled}>${child}</${el.tag}>`
  },
  'fc-input-table': el => {
    let labelWidth = '',
      formSize = ''
...
function buildElSelectChild(conf) {
  ...
  if (conf.options && conf.options.length) {
    children.push(
      `<el-option v-for="(item, index) in field${conf.formId}Options" :key="index" :label="item.label" :value="item.value" :disabled="item.disabled"></el-option>`
    )
  }
  return children.join('\n')
}
...
function buildElRadioGroupChild(conf) {
  ...
  if (conf.options && conf.options.length) {
    ...
    children.push(
      `<${tag} v-for="(item, index) in field${conf.formId}Options" :key="index" :label="item.value" :disabled="item.disabled" ${border}>{{item.label}}</${tag}>`
    )
  }
  return children.join('\n')
}
