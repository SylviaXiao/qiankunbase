          ...
          <a-row>
            <a-col :lg="6" :md="6" :sm="24">
              <a-form-item label="关键字" v-bind="itemLayout">
                <a-input placeholder="请输入手机号码或姓名" v-decorator="['teacherName']"></a-input>
              </a-form-item>
            </a-col>
            <a-col :lg="6" :md="6" :sm="24">
            ...
            <!--<a-col :lg="6" :md="6" :sm="24">
              <a-form-item label="舞种" v-bind="itemLayout">
                <a-select allowClear v-decorator="['danceId']" placeholder="请选择舞种">
                  <a-select-option :value="dance.danceId" v-for="(dance, danceIndex) in danceList" :key="danceIndex">
                    {{ dance.danceName }}
                  </a-select-option>
                </a-select>
              </a-form-item>
...
import { pageTeachers } from '@/api/common'
import { listEduDance } from '@/api/recep'
  ...
  data() {
    return {
      ...
      tableLoad: parameter => {
        ...
        return request(Object.assign(parameter, queryParam)).then(res => {
          let that = this
          this.$nextTick(() => {
            that.setNumber()
          })
  ...
  methods: {
    ...
    handleOk() {
      if (this.selectedRows.length == 0 && this.teaFields !== 'assistant') {
        ...
        if (this.multiple && this.teaFields == 'teacher') {
