...
export default {
  ...
  methods: {
    ...
    chooseModalCreator(itemObj) {
      this.multiple = itemObj.multiple
      return (
        <a-col lg={8} md={12} sm={24} style={this.isShowAll || this.isShowAll1 || itemObj.show ? '' : 'display:none;'}>
    ...
    dateCreator(itemObj) {
      ...
      return (
        <a-col lg={8} md={12} sm={24} style={this.isShowAll || this.isShowAll1 || itemObj.show ? '' : 'display:none;'}>
          <a-form-item labelCol={this.formItemLayout.labelCol} wrapperCol={this.formItemLayout.wrapperCol} label={itemObj.label}>
            <a-range-picker
              disabled-date={itemObj.oldYear ? this.disabledDate.bind() : null}
              style="width:100%"
              onPanelChange={this.handlePanelChange2.bind()}
              mode={itemObj.mode}
    ...
    _normalizeDate(obj) {
      for (let k in obj) {
        //处理时间函数
        if ((obj[k] instanceof moment && k == 'signDate') || k == 'day' || k == 'singleDate') {
