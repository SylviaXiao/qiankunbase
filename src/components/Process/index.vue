...
export default {
  ...
  data() {
    ...
    if (typeof this.conf === 'object' && this.conf !== null) {
      ...
      data.content = this.conf.properties.initiator.data ? this.conf.properties.initiator.data.map(t => t.name) : this.conf.properties.initiator
    }
    return {
      data, // 流程图数据
  ...
  methods: {
    ...
    onPropEditConfirm(value, content) {
      ...
      if (NodeUtils.isConditionNode(this.activeData)) {
        value.priority !== oldProp.priority && NodeUtils.resortPrioByCNode(this.activeData, oldProp.priority, this.data)
      ...
      //       this.$nextTick(() => {
      //   this.$emit('initiatorChange', this.initiator, value)
      // })
      this.onClosePanel()
      this.forceUpdate()
    },
    ...
    return (
      <div class="flow-container">
        <div class="scale-slider">
          <i class="btn  el-icon-minus" onClick={this.changeScale.bind(this, -this.step)} />
          <span style="font-size:14px;">{this.scaleVal}%</span>
          <i class="btn  el-icon-plus " onClick={this.changeScale.bind(this, this.step)} />
        </div>
        <FlowCard
        ...
        />
        <PropPanel value={this.activeData} processData={this.data} onConfirm={this.onPropEditConfirm} onCancel={this.onClosePanel} />
      </div>
    )
  }
}
