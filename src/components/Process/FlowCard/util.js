...
// const hasBranch = data => true
// {
//   console.log(data)
// if (assigneeType == 'branch') {
// }

// if (data.branchIdList && data.branchIdList.length === 0) {
//   return false
// } else if (data.positionIdList && data.positionIdList.data && data.positionIdList.data.length === 0) {
//   return false
// } else {
//   return true
// }
...
export class NodeUtils {
  ...
  static setDefaultCondition(cnode, processData) {
    ...
    const hasCondition = node =>
      node.properties &&
      (node.properties.initiator || node.properties.initiatorType || node.properties.initiatorBranch || !isEmptyArray(node.properties.conditions))
  ...
  static checkNode(node, parent) {
    ...
    this.isConditionNode(node) &&
      ...
      !props.initiator &&
      !props.initiatorType &&
      !props.initiatorBranch &&
      isEmptyArray(props.conditions) &&
      (valid = false)

    const customSettings = ['myself', 'optional', 'director']
    this.isApproverNode(node) && !customSettings.includes(props.assigneeType) && isEmptyArray(props.menbers) && !props.branch && (valid = false)
    return valid
  }
  /**
   * 判断所有节点是否信息完整
