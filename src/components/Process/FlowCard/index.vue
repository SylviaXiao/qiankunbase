    ...
    <section class={classList.join(' ')} onClick={this.eventLancher.bind(ctx, 'edit', conf)}>
      <header class="header">
        <div class="title-box" style="height: 100%;width:190px;">
          {isApprNode && <i class="iconfont iconshenpi" style="font-size:12px;color:white;margin-right:4px;" />}
          {isCopyNode && <i class="el-icon-s-promotion" style="font-size:12px;color:white;margin-right:4px;" />}
        ...
        <div class="actions" style="margin-right:4px;">
          <i class="el-icon-close icon" onClick={this.eventLancher.bind(ctx, 'deleteNode', conf, ctx.data)} />
        </div>
      </header>
      <div class="body">
        <span class="text">{conf.content}</span>
        <div class="icon-wrapper right">
          <i class="el-icon-arrow-right icon right-arrow" />
        </div>
      </div>
    </section>
  )
  ...
  condition: function(ctx, conf, h) {
    ...
    return (
      <section class="flow-path-card condition" onClick={this.eventLancher.bind(ctx, 'edit', conf)}>
        ...
        <div class="icon-wrapper left" onClick={ctx.eventLancher.bind(ctx, 'increasePriority', conf, ctx.data)}>
          <i class="el-icon-arrow-left icon left-arrow" />
        </div>
        <div class="icon-wrapper right" onClick={ctx.eventLancher.bind(ctx, 'decreasePriority', conf, ctx.data)}>
          <i class="el-icon-arrow-right icon right-arrow" />
        </div>
      </section>
    )
          ...
          <div class="condition-box">
            <div>
              <div class="condition-icon" onClick={ctx.eventLancher.bind(ctx, 'addApprovalNode', data, isBranch)}>
                <i class="iconfont el-icon-s-check" style="vertical-align: middle;" />
              </div>
              审批人
            </div>
            ...
            <div>
              <div class="condition-icon" onClick={ctx.eventLancher.bind(ctx, 'addCopyNode', data, isBranch)}>
                <i class="iconfont el-icon-s-promotion" style="vertical-align: middle;" />
              </div>
              抄送人
            </div>
            ...
            <div>
              <div class="condition-icon" onClick={this.eventLancher.bind(ctx, 'appendBranch', data, isBranch)}>
                <a-icon type="branches" class="iconfont" style="vertical-align: middle;" />
              </div>
              条件分支
            </div>
          </div>
  ...
  if (isCondition(data)) {
    return (
      <div class="col-box">
        ...
        <div class="top-cover-line" />
        <div class="bottom-cover-line" />
        {selfNode}
        {branchNode}
