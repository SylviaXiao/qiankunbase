...
function buildRules(conf, ctx) {
  ...
  if (conf.required) {
    ...
    conf.tag === 'fc-org-select' ? rules.push(setFcOrgSelectRule(conf, ctx)) : rules.push({ required: true, type, message, trigger: trigger[conf.tag] })
  }
  // 自定义正则匹配
...
const layouts = {
  colFormItem: function(conf, h, ctx, isList = false) {
    ...
    if (isList) {
      var tableTitle = (
        <el-col span={24} style="font-size: 14px;padding:6px 0px;margin-bottom: 4px;border-bottom: 1px solid #e5e5e5;">
          {conf.label}
        </el-col>
      )
...
export default {
  ...
  render(h) {
    return (
      <div class="preview-container" style={'width:' + this.containerWidth + '%;height: 100%;'}>
        <el-row gutter={this.confGlobal.gutter} style="padding: 20px 20px 0;">
          {this.buildForm(h)}
        </el-row>
