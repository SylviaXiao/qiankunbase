...
export default {
  ...
  methods: {
    init() {
      ...
      if (that.procdefId) {
        //首次发起流程
        workFlowProcdefGet({ id: this.procdefId }).then(function(data) {
          ...
          if (that.config.basicSetting.formType == that.formTypes.Online.value) {
            ...
            if (that.procdef) {
              if (that.procinstId) {
                workflowProcinstGet(that.procinstId).then(res => {
                  that.procinst = res.data
                  that.getFormData(JSON.parse(res.data.flowVal), formOperatesMap)
                })
              } else {
    ...
    getFormData(formData, formOperatesMap) {
      this.confGlobal = formData
      this.confGlobal.fields = this.formatEdit(formData.fields, formOperatesMap)
      this.confGlobal = formData
      this.formData = this.confGlobal
      this.confGlobal.fields.forEach((item, index) => {
        if (item.defaultValue) {
          this.formData[item.vModel] = JSON.parse(JSON.stringify(item.defaultValue))
        }
        if (item.tag == 'el-date-picker') {
          let date = moment(new Date()).format('YYYY-MM-DD')
          this.formData[item.vModel] = this.formData[item.vModel] || date
        }
      })
    },
    getFlowLine() {
