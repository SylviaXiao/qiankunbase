...
export function hasBtnPerm(Vm, el) {
  return new Promise((resolve, reject) => {
    ...
    store.getters.roles.permissionList && store.getters.roles.permissionList[0] == '*' ? (Vm.superAdmin = true) : (Vm.superAdmin = false)
    const btnObj = new Object()
    if (Array.isArray(store.getters.roles.permissionList))
